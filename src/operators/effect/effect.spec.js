...
describe('effect', () => {
    ...
    testCasesForData([1, 2, 3, 4])(
        'should call the side effect for each value yielded when given a(n) %s',
        (type, dataSource) => {
    ...
    testCasesForData([1, 2, 3, 4])(
        'should only call the side effect when the consumer requests the next value when given a(n) %s',
        (type, dataSource) => {
            const sideEffectFunction = jest.fn();
    ...
    testCasesForData([1, 2, 3])(
        'should return an iterable with the same values that it received when given a(n) %s',
        (type, data) => {
